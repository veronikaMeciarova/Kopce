package com.example.vm.kopce;
    ...
    //import com.example.vm.comoflyer.Panorama;
...
public class CannyEdgeDetector {
    ...
    private static float[][] panoramaMatrix;
    private static Bitmap edgesImage;
     ...
     * is called.
     *
     * @param matrix a source of luminance data
     */

    public void setSourceImage(float[][] matrix) {
        panoramaMatrix = matrix;
    }
    ...
    public void process() {
        width = panoramaMatrix[0].length; //6400
        height = panoramaMatrix.length; //800
    ...
    //vyrobi ciernobielu farbu - luminance = jas
...
//            for (int j=0; j<width; j++) {
//                int val = -1;
//                float dist = panoramaMatrix[i][j];
//                if (dist == 180000) {
//                    val = 0;
//                } else {
//                    val = (int)(dist/scale);
//                }
//                data[pos] = luminance(val,val,val);
//                pos++;
//            }
//        }
...
//        }
        ...
        for (int i=0; i<pixels.length; i++) {
            if (pixels[i] == -16777216) {
                ...
                pixels[i] = -65281; //ruzova
            }
        }
        //TREBA ESTE OTOCIT POLE
        Bitmap.Config conf = Bitmap.Config.ARGB_4444;
        edgesImage = Bitmap.createBitmap(width, height, conf);
        edgesImage.setPixels(pixels,0,width,0,0,width,height);

    }

    public static Bitmap getPanoramaEdges() { //main of this class
        //create the detector
        CannyEdgeDetector detector = new CannyEdgeDetector();
        ...
        //detector.setSourceImage(panorama.getLinearizedDistanceMatrix());
        detector.setSourceImage(pan);
        detector.process();
